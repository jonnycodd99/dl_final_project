...
def compare(images: list, titles: list = ['']) -> None:
    '''Displays multiple images with a legend.'''
    nimage = len(images)
    if len(titles) == 1:
    ...
    # Define the colors and labels for the legend based on the key
    key = {
    ...
    patches = [mpatches.Patch(color=np.array(k)/255, label=v) for k, v in key.items()]
